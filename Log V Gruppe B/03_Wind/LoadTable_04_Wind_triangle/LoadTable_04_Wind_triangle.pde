...
void settings() {
  ...
  int w=padding+24*(rectlength+spacing)+padding;
...
void setup() {
  ...
  for (TableRow row : table.rows()) { //short  for (int i = 0; i < zeilen.length; i++ ){int wert = zeilen[i];}
    ...
    if (windspeed !=windspeed) {
      ...
      translate(posX+rectlength/2, posY+rectlength/2);
      rotate(radians(winddir));
      translate(-mappedWindspeed/2,0);
      triangle(0,0,mappedWindspeed,rectheight/2,0,rectheight);
